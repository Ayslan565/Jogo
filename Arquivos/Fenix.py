...
class Fenix(Inimigo): 
    ...
    def __init__(self, x, y, velocidade=3.0): 
        ...
        if Fenix.sprites_originais is None: 
            caminhos = [
            ...
            for path in caminhos:
                ...
                try:
                    ...
                    print(f"DEBUG(Fenix): Erro ao carregar o sprite da Fênix: {full_path} - {e}")
                    placeholder = pygame.Surface(Fenix.tamanho_sprite_definido, pygame.SRCALPHA)
                    pygame.draw.rect(placeholder, (255, 165, 0), (0, 0, Fenix.tamanho_sprite_definido[0], Fenix.tamanho_sprite_definido[1])) 
                    Fenix.sprites_originais.append(placeholder)
        ...
        self.sprites = Fenix.sprites_originais 
        self.sprite_index = 0 
        self.tempo_ultimo_update_animacao = pygame.time.get_ticks() 
        ...
        self.attack_cooldown = 2.0 # Em segundos
        self.last_attack_time = time.time() - self.attack_cooldown # Permite atacar mais cedo

        # self.facing_right é herdado e gerenciado pela classe base Inimigo
    ...
    def atualizar_animacao(self):
        ...
        # e a troca de sprites com base em self.sprites, self.sprite_index, etc.
    ...
    def update(self, player, projeteis_inimigos_ref=None, tela_largura=None, altura_tela=None):
        ...
        if self.esta_vivo():
            ...
            if self.is_attacking:
                # Atualiza a posição da hitbox de ataque (caso a Fênix se mova durante o ataque)
                self.attack_hitbox.center = self.rect.center 

                if current_time_ataque - self.attack_timer >= self.attack_duration:
                    self.is_attacking = False
                    self.hit_by_player_this_attack = False 
                    # print(f"DEBUG(Fenix): Fênix terminou animação de ataque.")
                else:
    ...
    def desenhar(self, surface, camera_x, camera_y):
        ...
        #     pygame.draw.rect(surface, (255, 100, 0, 150), debug_hitbox_rect_onscreen, 1)
