...
try:
    ...
    class Weapon:
        def __init__(self, name="Arma Desconhecida", ui_icon_path=None, **kwargs):
            self.name = name
            self.ui_icon_path = ui_icon_path if ui_icon_path else DEFAULT_WEAPON_ICON_PATH
            self._base_name = name
            self.level = 1
            # Definindo atributos padrão que podem ser esperados
            self.damage = kwargs.get('damage', 10)
            self.attack_range = kwargs.get('attack_range', 50)
            self.cooldown = kwargs.get('cooldown', 0.5)
            self.hitbox_width = kwargs.get('hitbox_dimensions', (0,0))[0]
            self.hitbox_height = kwargs.get('hitbox_dimensions', (0,0))[1]
            self.hitbox_offset_x = kwargs.get('hitbox_offset', (0,0))[0]
            self.hitbox_offset_y = kwargs.get('hitbox_offset', (0,0))[1]
            self.attack_animation_sprites = kwargs.get('attack_animation_sprites', [])
            self.attack_animation_speed = kwargs.get('attack_animation_speed', 100)
...
# Bloco para configurar caminhos de importação
project_root_for_import = None
try:
    current_file_dir_for_import = os.path.dirname(os.path.abspath(__file__))
    temp_root = os.path.dirname(current_file_dir_for_import) # Jogo/Arquivos

    if "Jogo" in temp_root:
        while os.path.basename(temp_root) != "Jogo":
    ...
    from Arquivos.Armas.CajadoDaFixacaoAmetista import CajadoDaFixacaoAmetista
    from Arquivos.Armas.CajadoDaSantaNatureza import CajadoDaSantaNatureza
    from Arquivos.Armas.LivroDosImpuros import LivroDosImpuros
    from Arquivos.Armas.EspadaBrasas import EspadaBrasas
except ImportError as e:
    print(f"ALERTA(BarraInventario Imports): Falha ao importar classes de armas para teste: {e}.")
    AdagaFogo = EspadaBrasas = CajadoDaFixacaoAmetista = CajadoDaSantaNatureza = LivroDosImpuros = None
...
class BarraInventario:
    ...
    def _calcular_rects_painel_central(self):
        ...
        rect_top = pygame.Rect(0, 0, slot_w, slot_h); rect_top.center = (centro_x_tela, centro_y_tela - offset_do_centro_slots)
        rect_direita = pygame.Rect(0, 0, slot_w, slot_h); rect_direita.center = (centro_x_tela + offset_do_centro_slots, centro_y_tela)
        rect_bottom = pygame.Rect(0, 0, slot_w, slot_h); rect_bottom.center = (centro_x_tela, centro_y_tela + offset_do_centro_slots)
        rect_esquerda = pygame.Rect(0, 0, slot_w, slot_h); rect_esquerda.center = (centro_x_tela - offset_do_centro_slots, centro_y_tela)
        ...
        altura_area_titulo = 45; altura_area_nome_arma = 40; altura_area_dinheiro = 35
    ...
    def _get_project_root(self) -> str:
        ...
        if os.path.isdir(os.path.join(project_root_candidate, "Sprites")):
    ...
    def _load_image_from_path(self, image_path_relativo_ao_projeto: str, cache_key_prefix="icon", novo_tamanho=None) -> pygame.Surface | None:
        ...
        if not novo_tamanho:
            ...
            tamanho_icone_final = (max(1, tamanho_usado_para_cache[0] - padding_icone*2), max(1, tamanho_usado_para_cache[1] - padding_icone*2))
    ...
    def _carregar_icone_arma(self, arma_instancia: Weapon, tamanho_personalizado=None) -> pygame.Surface | None:
        ...
        if caminho_icone_especifico:
            icone = self._load_image_from_path(caminho_icone_especifico, cache_key_prefix=f"icon_{nome_arma_para_icone.replace(' ', '_')}", novo_tamanho=tamanho_personalizado)
            if icone: return icone
        
        icone_default = self._load_image_from_path(DEFAULT_WEAPON_ICON_PATH, cache_key_prefix="default_icon", novo_tamanho=tamanho_personalizado)
        if icone_default: return icone_default

        if placeholder_cache_key in self._icon_cache: return self._icon_cache[placeholder_cache_key]
        placeholder_icone = self._criar_icone_placeholder_arma(nome_arma_para_icone, tamanho_icone=tamanho_ph_interno)
        if placeholder_icone: self._icon_cache[placeholder_cache_key] = placeholder_icone
    ...
    def _criar_icone_placeholder_arma(self, nome_arma="?", tamanho_icone=None):
        if tamanho_icone is None or not (isinstance(tamanho_icone, tuple) and len(tamanho_icone) == 2):
            ...
            tamanho_icone = (max(1,self.slot_tamanho_hud[0] - padding_default*2), max(1,self.slot_tamanho_hud[1] - padding_default*2))
        ...
        try:
            ...
            texto_ph = nome_arma[0].upper() if nome_arma else "?"
    ...
    def handle_input(self, evento, jogador_ref):
        ...
        if evento.type == pygame.KEYDOWN:
            ...
            if 0 <= novo_slot_selecionado_teclado < self.num_slots_painel:
                ...
                if arma_para_equipar_teclado: jogador_ref.equip_weapon(arma_para_equipar_teclado)
                return False
        elif evento.type == pygame.MOUSEBUTTONDOWN and self.visao_inventario_aberta and evento.button == 1:
            ...
            if self.rect_painel_fundo.collidepoint(mouse_pos):
                for i, slot_rect in enumerate(self.slot_rects_painel):
                    if slot_rect.collidepoint(mouse_pos):
                        ...
                        if arma_para_equipar_clique: jogador_ref.equip_weapon(arma_para_equipar_clique)
                        return True
                return True
            else:
    ...
    def desenhar(self, tela: pygame.Surface, jogador_ref):
        ...
        if not self.visao_inventario_aberta: return
        ...
        titulo_rect = titulo_surface.get_rect(centerx=self.rect_painel_fundo.centerx, top=self.rect_painel_fundo.top + self.espacamento_hud + 5)
        tela.blit(titulo_surface, titulo_rect)

        cx_tela, cy_tela = self.largura_tela_original // 2, self.altura_tela_original // 2
        offset_losango_visual = 80
        tamanho_icone_central_arma = (int(offset_losango_visual * 0.85), int(offset_losango_visual * 0.85))
        p_sup, p_dir, p_inf, p_esq = (cx_tela, cy_tela - offset_losango_visual), (cx_tela + offset_losango_visual, cy_tela), (cx_tela, cy_tela + offset_losango_visual), (cx_tela - offset_losango_visual, cy_tela)
        ...
        if 0 <= idx_painel_sel < 4:
            if idx_painel_sel == 0: pygame.draw.line(tela, cor_ponta_sel_losango, p_esq, p_sup, esp_ponta_sel_losango); pygame.draw.line(tela, cor_ponta_sel_losango, p_sup, p_dir, esp_ponta_sel_losango)
            elif idx_painel_sel == 1: pygame.draw.line(tela, cor_ponta_sel_losango, p_sup, p_dir, esp_ponta_sel_losango); pygame.draw.line(tela, cor_ponta_sel_losango, p_dir, p_inf, esp_ponta_sel_losango)
            elif idx_painel_sel == 2: pygame.draw.line(tela, cor_ponta_sel_losango, p_dir, p_inf, esp_ponta_sel_losango); pygame.draw.line(tela, cor_ponta_sel_losango, p_inf, p_esq, esp_ponta_sel_losango)
            elif idx_painel_sel == 3: pygame.draw.line(tela, cor_ponta_sel_losango, p_inf, p_esq, esp_ponta_sel_losango); pygame.draw.line(tela, cor_ponta_sel_losango, p_esq, p_sup, esp_ponta_sel_losango)

        for i, slot_rect_painel_item in enumerate(self.slot_rects_painel):
            fundo_slot_surf = pygame.Surface(self.slot_tamanho_hud, pygame.SRCALPHA); fundo_slot_surf.fill(COR_FUNDO_SLOT)
            tela.blit(fundo_slot_surf, slot_rect_painel_item.topleft)
            arma_no_slot_painel = armas_do_jogador[i] if i < len(armas_do_jogador) else None
            cor_borda_atual_slot, espessura_borda_atual_slot = (COR_BORDA_EQUIPADA, 3) if arma_no_slot_painel and arma_no_slot_painel is arma_equipada_jogador else ((COR_BORDA_SELECIONADA, 3) if i == self.item_selecionado_index_painel else (COR_BORDA_SLOT, 2))
            ...
            if arma_no_slot_painel:
                ...
                if i == self.item_selecionado_index_painel: nome_arma_para_exibir_painel = arma_no_slot_painel.name
            
            if i == self.item_selecionado_index_painel and not arma_no_slot_painel: nome_arma_para_exibir_painel = "Vazio"
        ...
        if nome_arma_para_exibir_painel:
            ...
            nome_arma_rect_detalhe = nome_arma_surf_detalhe.get_rect(centerx=self.rect_painel_fundo.centerx, centery=pos_y_nome_arma)
        ...
        rect_dinheiro_jogador = surface_dinheiro_jogador.get_rect(left=self.rect_painel_fundo.left + self.espacamento_hud + 10, bottom=self.rect_painel_fundo.bottom - self.espacamento_hud - 5)
    ...
    def _desenhar_barra_rapida_hud(self, tela: pygame.Surface, jogador_ref):
        ...
        for i, slot_rect_hud_item in enumerate(self.slot_rects_hud):
            ...
            fundo_slot_hud_surf = pygame.Surface(self.slot_tamanho_hud, pygame.SRCALPHA); fundo_slot_hud_surf.fill(COR_FUNDO_SLOT)
            tela.blit(fundo_slot_hud_surf, slot_rect_hud_item.topleft)
            arma_neste_slot_hud_item = armas_para_hud[i] if i < len(armas_para_hud) else None
            cor_borda_hud_slot, espessura_borda_hud_slot = (COR_BORDA_EQUIPADA, 3) if arma_neste_slot_hud_item and arma_neste_slot_hud_item is arma_equipada_jogador_hud else (COR_BORDA_SLOT, 2)
...
if __name__ == '__main__':
    pygame.init()
    LARGURA_TELA_TESTE, ALTURA_TELA_TESTE = 800, 600
    tela_teste = pygame.display.set_mode((LARGURA_TELA_TESTE, ALTURA_TELA_TESTE))
    pygame.display.set_caption("Teste da Barra de Inventário - Múltiplas Armas")
    ...
    class MockJogador:
        def __init__(self):
            ...
            # Adiciona as armas importadas para teste
            if AdagaFogo: self.owned_weapons[0] = AdagaFogo()
            if CajadoDaFixacaoAmetista: self.owned_weapons[1] = CajadoDaFixacaoAmetista()
            if CajadoDaSantaNatureza: self.owned_weapons[2] = CajadoDaSantaNatureza()
            if LivroDosImpuros: self.owned_weapons[3] = LivroDosImpuros()

            # Equipa a primeira arma da lista que não for None
            for arma in self.owned_weapons:
                if arma:
                    self.equip_weapon(arma)
                    break
        
        def equip_weapon(self, weapon):
            self.current_weapon = weapon
            print(f"MockJogador: Equipou '{weapon.name}'")

    jogador_teste = MockJogador()
    barra_inventario_teste = BarraInventario(10, ALTURA_TELA_TESTE - 80, LARGURA_TELA_TESTE, ALTURA_TELA_TESTE)
    ...
    while rodando_teste:
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN and evento.key == pygame.K_TAB:
                barra_inventario_teste.toggle_visao_inventario(jogador_teste)
            
            barra_inventario_teste.handle_input(evento, jogador_teste)

        tela_teste.fill((30, 30, 30))
        barra_inventario_teste.desenhar(tela_teste, jogador_teste)
        pygame.display.flip()
        clock.tick(60)
