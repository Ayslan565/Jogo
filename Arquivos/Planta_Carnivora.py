    ...
    class Inimigo(pygame.sprite.Sprite):
        ...
        def desenhar(self, janela, camera_x, camera_y):
            ...
            if self.hp < self.max_hp and self.hp > 0:
                ...
                pygame.draw.rect(janela, (139,0,0), (bar_x, bar_y, bar_width, bar_height), border_radius=3) 
                pygame.draw.rect(janela, (34,139,34), (bar_x, bar_y, current_bar_width, bar_height), border_radius=3) 
                pygame.draw.rect(janela, (220,220,200), (bar_x, bar_y, bar_width, bar_height), 1, border_radius=3) 
...
class Mae_Natureza(Inimigo):
    ...
    som_dano_mae = None
    som_morte_mae = None
    # som_spawn_mae = None # Opcional
    sons_carregados = False

    @staticmethod
    def _carregar_som_mae_natureza(caminho_relativo):
        current_file_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(current_file_dir) # Assume que Mae_Natureza.py está em subpasta
    ...
    def _carregar_lista_sprites_estatico(caminhos, lista_destino, tamanho, tipo_animacao):
        current_file_dir = os.path.dirname(os.path.abspath(__file__))
        game_root_dir = os.path.dirname(current_file_dir) # Assume que Mae_Natureza.py está em subpasta
        ...
        if not lista_destino:
            ...
            placeholder = pygame.Surface(tamanho, pygame.SRCALPHA)
            pygame.draw.rect(placeholder, (20,100,20), placeholder.get_rect()) 
            lista_destino.append(placeholder)
    ...
    def carregar_recursos_mae_natureza():
        ...
        if Mae_Natureza.sprites_andar_carregados is None:
            ...
            # Exemplo para sprites de ataque, se existirem
        ...
        if not Mae_Natureza.sons_carregados:
            # Mae_Natureza.som_ataque_mae = Mae_Natureza._carregar_som_mae_natureza("Sons/Mae_Natureza/ataque_natureza.wav") # CRIE ESTES ARQUIVOS
            # Mae_Natureza.som_dano_mae = Mae_Natureza._carregar_som_mae_natureza("Sons/Mae_Natureza/dano_natureza.wav")
            # Mae_Natureza.som_morte_mae = Mae_Natureza._carregar_som_mae_natureza("Sons/Mae_Natureza/morte_natureza.wav")
    ...
    def __init__(self, x, y, velocidade=0.8): 
        ...
        # self.intervalo_animacao_atacar = 200 # Se tiver animação de ataque
        ...
        self.attack_hitbox_size = (self.largura, self.altura) # Ataque de área ao redor dela
        self.attack_hitbox = pygame.Rect(0, 0, 0, 0) 
    ...
    def atacar(self, player):
        ...
        if self.esta_vivo() and not self.is_attacking and (current_time - self.last_attack_time >= self.attack_cooldown):
            ...
            if distancia_ao_jogador <= self.attack_range:
                ...
                # A hitbox de ataque é o próprio rect da Mãe Natureza para um ataque de área
                self.attack_hitbox = self.rect.copy() 
    ...
    def update(self, player, outros_inimigos=None, projeteis_inimigos_ref=None, tela_largura=None, altura_tela=None, dt_ms=None):
        ...
        # Lógica de movimento e animação base
        if not self.is_attacking:
            try:
                # Passa 'outros_inimigos' para a classe base
                super().update(player, outros_inimigos, projeteis_inimigos_ref, tela_largura, altura_tela, dt_ms)
            except TypeError: 
                try:
                    # Fallback se a classe base não aceitar 'outros_inimigos'
                    super().update(player, projeteis_inimigos_ref, tela_largura, altura_tela, dt_ms)
                except TypeError:
                     # Fallback se a classe base não aceitar 'dt_ms'
                    super().update(player, outros_inimigos, projeteis_inimigos_ref, tela_largura, altura_tela)
        else:
            self.atualizar_animacao() # Se está atacando, só anima

        if self.esta_vivo():
            current_time_ataque = time.time() 

            if self.is_attacking:
                # A hitbox de ataque é o próprio rect da Mãe Natureza, atualizado com a posição dela
                self.attack_hitbox = self.rect.copy()
                ...
                if current_time_ataque - self.attack_timer >= self.attack_duration:
                    ...
                    # Dano de área: verifica colisão com o jogador usando a hitbox de ataque
                    if not self.hit_by_player_this_attack and self.attack_hitbox.colliderect(player.rect):
                        if player.vida.esta_vivo(): 
                            ...
                            # print(f"DEBUG(Mae_Natureza): Ataque de área acertou o jogador! Dano: {self.attack_damage}")
    ...
    def desenhar(self, surface, camera_x, camera_y):
        ...
        # Opcional: Desenhar a hitbox de ataque para debug (seria o próprio rect dela)
        # if self.is_attacking:
        #     debug_rect_onscreen = self.rect.move(-camera_x, -camera_y)
        #     pygame.draw.rect(surface, (34, 139, 34, 100), debug_rect_onscreen, 2) # Borda verde
