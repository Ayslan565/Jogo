...
try:
    ...
    class Inimigo(pygame.sprite.Sprite): # Placeholder
        def __init__(self, x, y, largura, altura, vida_maxima, velocidade, dano_contato, xp_value, sprite_path):
            ...
            self.sprite_path_base = sprite_path # Armazena o caminho do sprite base

            self.image = pygame.Surface((largura, altura), pygame.SRCALPHA)
            pygame.draw.rect(self.image, (0, 100, 0), (0, 0, largura, altura)) # Placeholder verde escuro para Goblin
...
class Goblin(Inimigo):
    ...
    sprites_atacar_carregados = None # Para animação de ataque
    tamanho_sprite_definido = (50, 60) # Ajuste o tamanho conforme necessário

    def __init__(self, x, y, velocidade=2.0): 
        print(f"DEBUG(Goblin): Inicializando Goblin em ({x}, {y}) com velocidade {velocidade}.")

        goblin_hp = 40
        goblin_contact_damage = 6
        goblin_xp_value = 15
        sprite_path_principal_andar = "Sprites/Inimigos/Goblin/Goblin_Andar1.png" 
        # sprite_path_principal_atacar = "Sprites/Inimigos/Goblin/Goblin_Atacar1.png" # Se tiver
        ...
        if Goblin.sprites_andar_carregados is None:
            ...
            self._carregar_lista_sprites(caminhos_andar, Goblin.sprites_andar_carregados, Goblin.tamanho_sprite_definido, "Andar")
        ...
        if Goblin.sprites_atacar_carregados is None:
            ...
            if caminhos_atacar: # Só carrega se houver caminhos definidos
                 self._carregar_lista_sprites(caminhos_atacar, Goblin.sprites_atacar_carregados, Goblin.tamanho_sprite_definido, "Atacar")
            else: # Se não houver sprites de ataque, pode usar os de andar ou um frame parado
                if Goblin.sprites_andar_carregados:
                    Goblin.sprites_atacar_carregados = [Goblin.sprites_andar_carregados[0]] # Usa o primeiro frame de andar como placeholder de ataque
                else: # Fallback extremo
                    placeholder_ataque = pygame.Surface(Goblin.tamanho_sprite_definido, pygame.SRCALPHA)
                    pygame.draw.rect(placeholder_ataque, (0,80,0), (0,0,Goblin.tamanho_sprite_definido[0],Goblin.tamanho_sprite_definido[1]))
        ...
        super().__init__(x, y, 
                         ...
                         goblin_xp_value, sprite_path_principal_andar)

        self.sprites = Goblin.sprites_andar_carregados # Começa com animação de andar
        self.sprite_index = 0 
        self.tempo_ultimo_update_animacao = pygame.time.get_ticks() 
        self.intervalo_animacao_andar = 150 
        self.intervalo_animacao_atacar = 100 # Pode ser diferente para o ataque
        self.intervalo_animacao = self.intervalo_animacao_andar # Intervalo atual

        self.is_attacking = False 
        self.attack_duration = 0.5 # Duração da animação de ataque
        self.attack_timer = 0.0 
        self.attack_damage = 10 # Dano do ataque do Goblin
        self.attack_range = 50  # Curto alcance para ataque corpo a corpo
        ...
        for path in caminhos:
            ...
            try:
                if os.path.exists(full_path):
                    ...
                    print(f"DEBUG(Goblin): Aviso: Sprite {tipo_animacao} do Goblin não encontrado: {full_path}. Usando placeholder.")
                    placeholder = pygame.Surface(tamanho, pygame.SRCALPHA)
                    pygame.draw.rect(placeholder, (0, 100, 0), (0, 0, tamanho[0], tamanho[1]))
                    lista_destino.append(placeholder)
            except pygame.error as e:
                print(f"DEBUG(Goblin): Erro ao carregar o sprite {tipo_animacao} do Goblin: {full_path} - {e}")
                placeholder = pygame.Surface(tamanho, pygame.SRCALPHA)
                pygame.draw.rect(placeholder, (0, 100, 0), (0, 0, tamanho[0], tamanho[1]))
                lista_destino.append(placeholder)
        
        if not lista_destino: # Fallback se nenhum sprite for carregado para esta animação
            print(f"DEBUG(Goblin): Aviso: Nenhum sprite de {tipo_animacao} do Goblin carregado. Usando placeholder padrão.")
            placeholder = pygame.Surface(tamanho, pygame.SRCALPHA)
            pygame.draw.rect(placeholder, (0, 100, 0), (0, 0, tamanho[0], tamanho[1]))
            lista_destino.append(placeholder)
    ...
    def atacar(self, player):
        ...
        if self.esta_vivo() and not self.is_attacking and (current_time - self.last_attack_time >= self.attack_cooldown):
            ...
            if distancia_ao_jogador <= self.attack_range: 
                ...
                self.hit_by_player_this_attack = False 
                self.sprites = Goblin.sprites_atacar_carregados # Muda para animação de ataque
                self.intervalo_animacao = self.intervalo_animacao_atacar
                self.sprite_index = 0 # Reinicia o índice da animação de ataque
                print(f"DEBUG(Goblin): Goblin iniciando ataque! Dist: {distancia_ao_jogador:.0f}")
    ...
    def update(self, player, projeteis_inimigos_ref=None, tela_largura=None, altura_tela=None):
        ...
        if self.esta_vivo():
            ...
            if self.is_attacking:
                # Atualiza a posição da hitbox de ataque
                if self.facing_right:
                    hitbox_x = self.rect.right
                else:
                    hitbox_x = self.rect.left - self.attack_hitbox_largura
                ...
                if current_time_ataque - self.attack_timer >= self.attack_duration:
                    self.is_attacking = False 
                    self.hit_by_player_this_attack = False 
                    self.sprites = Goblin.sprites_andar_carregados # Volta para animação de andar
                    self.intervalo_animacao = self.intervalo_animacao_andar
                    self.sprite_index = 0 # Reinicia o índice da animação de andar
                else:
                    if not self.hit_by_player_this_attack and \
                       self.attack_hitbox.colliderect(player.rect):
                        if player.vida.esta_vivo(): 
                            ...
                            print(f"DEBUG(Goblin): Goblin atacou o jogador! Dano: {self.attack_damage}")
            
            if not self.is_attacking:
                self.sprites = Goblin.sprites_andar_carregados # Garante que está usando sprites de andar
                self.intervalo_animacao = self.intervalo_animacao_andar
                self.atacar(player)
        
        # self.atualizar_animacao() # Já é chamado em super().update()

    # O método desenhar é herdado da classe Inimigo.
    # Se precisar de algo específico para o Goblin, pode sobrescrevê-lo.
    ...
    #         pygame.draw.rect(surface, (0, 255, 0, 100), debug_hitbox_rect_onscreen, 1) # Verde para hitbox do Goblin
