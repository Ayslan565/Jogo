...
class Inimigo(pygame.sprite.Sprite): 
    ...
    def _carregar_sprite(self, path, tamanho):
        ...
        # Tenta construir um caminho relativo à raiz do projeto se "Sprites" estiver lá
        project_root_guess = os.path.dirname(base_dir) # Assume que este arquivo está em uma subpasta
        
        # Verifica se o path já é absoluto ou começa com "Sprites" (indicando que já está formatado)
        if os.path.isabs(path) or path.lower().startswith("sprites"):
            full_path = os.path.join(project_root_guess, path.replace("\\", "/"))
        else: # Se for um path mais simples, assume que é relativo a uma pasta 'Sprites' dentro de base_dir
             full_path = os.path.join(base_dir, "Sprites", path.replace("\\", "/")) # Ex: "Fantasma/idle.png"
             if not os.path.exists(full_path): # Tenta relativo direto ao base_dir
                  full_path = os.path.join(base_dir, path.replace("\\", "/"))


        if not os.path.exists(full_path):
            # print(f"ALERTA(Inimigo._carregar_sprite): Sprite não encontrado em '{full_path}'. Usando placeholder.")
        ...
        try:
            ...
            # print(f"ERRO(Inimigo._carregar_sprite): Falha ao carregar '{full_path}'. Usando placeholder.")
            img = pygame.Surface(tamanho, pygame.SRCALPHA)
            img.fill((255,0,255,100))
            return img
    ...
    def mover_em_direcao(self, alvo_x, alvo_y, dt_ms=None):
        if self.esta_vivo() and self.velocidade > 0:
            ...
            if dist > self.velocidade * fator_tempo: 
                ...
                self.rect.x += mov_x
                self.rect.y += mov_y
                self.x = float(self.rect.x) # Atualiza x, y da instância
...
class GerenciadorDeInimigos:
    def __init__(self, estacoes_obj, tela_largura: int, altura_tela: int,
        ...
        self.inimigos = pygame.sprite.Group() # Grupo para inimigos normais
        self.projeteis_inimigos = pygame.sprite.Group()
        self.grupo_chefe_ativo = pygame.sprite.GroupSingle() # Grupo para o chefe atual
        ...
        self._spawn_normal_pausado = False # Flag para pausar spawn de inimigos normais
        ...
        while not self.stop_spawn_thread_event.is_set(): 
            ...
            if agora - self.ultimo_spawn_controlado_pelo_thread >= intervalo_atual:
                if len(self.inimigos) < self.limite_inimigos: 
                    if self.estacoes and hasattr(self.estacoes, 'nome_estacao_atual'): # Ajustado para nome_estacao_atual
                        try:
    ...
    def _spawn_inimigo_especifico_da_estacao(self, estacao_nome, jogador, dt_ms=None): 
        ...
        mapa_estacao_inimigos = {
            ...
            "verao": ['fenix', 'urso', 'demonio'] # Alias para verão
        }
        if est_nome_lower in mapa_estacao_inimigos:
            for tipo_str in mapa_estacao_inimigos[est_nome_lower]:
                # Converte string para nome de classe (ex: 'fantasma' -> 'Fantasma')
                nome_classe = tipo_str.capitalize().replace("_d", "_D").replace("_n", "_N").replace("_c", "_C") # Ajusta capitalização
                if nome_classe == "Bonecodeneve": nome_classe = "BonecoDeNeve" # Casos especiais
    ...
    def criar_inimigo_aleatorio(self, tipo_inimigo_str, x, y, velocidade=1.0, dt_ms_para_init=None): 
        ...
        if ClasseInimigo:
            try: novo_inimigo = ClasseInimigo(x=x, y=y, velocidade=velocidade)
                ...
                except Exception: pass # print(f"ERRO criar_inimigo_aleatorio {tipo_inimigo_str}")
            except Exception: pass
    ...
    def update_inimigos(self, jogador, dt_ms=None): 
        ...
        for inimigo in list(self.inimigos): 
            if hasattr(inimigo, 'esta_vivo') and inimigo.esta_vivo():
                if hasattr(inimigo, 'update'):
                    ...
                    except TypeError: # Fallbacks como antes
        ...
        for ini_rem in remover:
            if hasattr(jogador, 'ganhar_xp') and callable(jogador.ganhar_xp) and hasattr(ini_rem, 'xp_value'):
                jogador.ganhar_xp(ini_rem.xp_value) 
    ...
    def limpar_todos_inimigos_normais(self):
        ...
        for proj in list(self.projeteis_inimigos): # Limpa projéteis também
            if hasattr(proj, 'kill'): proj.kill()
        self.projeteis_inimigos.empty()
        # print("DEBUG(GerenciadorDeInimigos): Inimigos normais e projéteis limpos.")
    ...
    def spawn_chefe_estacao(self, indice_estacao_chefe, posicao_mundo_spawn):
        ...
        if indice_estacao_chefe == 0: # Primavera -> ArvoreMaldita
            if ArvoreMaldita is not None:
                try:
                    # A classe ArvoreMaldita(x, y, velocidade)
                    # posicao_mundo_spawn é o centro da arena. Ajusta para topleft.
                    temp_arvore = ArvoreMaldita(0,0) # Instancia temporária para pegar o tamanho
                    ...
                    # print(f"ERRO(GerenciadorInimigos): Falha ao instanciar ArvoreMaldita: {e}")
                    pass
            # else: print("ALERTA(GerenciadorInimigos): Classe ArvoreMaldita não disponível.")
        # Adicionar 'elif indice_estacao_chefe == X:' para outros chefes
            ...
            if hasattr(chefe, 'update') and callable(chefe.update):
                try: 
                    ...
                    try: chefe.update(jogador, dt_ms)
                        ...
                        except Exception: pass # print(f"ERRO update_chefe: {e_uc}")
    ...
    def limpar_chefe_ativo(self):
        if self.grupo_chefe_ativo.sprite:
            ...
            self.grupo_chefe_ativo.empty()
            # print(f"DEBUG(GerenciadorDeInimigos): Chefe {type(chefe).__name__} limpo.")
            
    def stop_threads(self):
        self.stop_spawn_thread_event.set() 
        if self.spawn_controller_thread and self.spawn_controller_thread.is_alive():
            self.spawn_controller_thread.join(timeout=0.5) # Reduzido timeout
