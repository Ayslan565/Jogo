...
import os
import math
import time # Not strictly used in the provided snippet of Planta_Carnivora directly, but often useful
import random # Same as time
...
try:
    ...
    class InimigoBase(pygame.sprite.Sprite):
        ...
        def receber_dano(self, dano, fonte_dano_rect=None):
            ...
            # Simular flash de dano básico se não houver na base real
...
class Planta_Carnivora(InimigoBase):
    ...
    tamanho_sprite_definido = (60, 60) # Ajuste conforme o sprite real
    ...
    def carregar_recursos_planta_carnivora(): # Renomeado
        ...
        if not Planta_Carnivora.sons_carregados:
            # Descomente e ajuste os caminhos se tiver os arquivos de som
    ...
    def __init__(self, x, y, velocidade=0.5): # Plantas carnívoras podem ser lentas ou imóveis
        ...
        self.tempo_ultimo_update_animacao = pygame.time.get_ticks() # Inicializar tempo da animação

        self.is_attacking = False
        self.attack_duration = 0.8 # Duração da mordida/ataque em segundos
        self.attack_timer = 0.0 # Timestamp de quando o ataque começou
    ...
    def atacar(self, player):
        ...
        agora = pygame.time.get_ticks()
        # Calcular distância apenas se o jogador e seu rect existirem
    ...
    def update(self, player, outros_inimigos=None, projeteis_inimigos_ref=None, tela_largura=None, altura_tela=None, dt_ms=None):
        ...
        if self.is_attacking:
            self.atualizar_animacao() # Chama a animação da InimigoBase ou a sobrescrita
            ...
            if jogador_valido and not self.hit_player_this_attack_swing and \
               self.attack_hitbox.colliderect(player.rect):
                # print(f"DEBUG(Planta_Carnivora): Colisão de ataque detectada com jogador. Hitbox: {self.attack_hitbox}, Player rect: {player.rect}")
                player.receber_dano(self.attack_damage_especifico, self.rect) # Passar self.rect como fonte_dano_rect
            ...
            if agora - self.attack_timer >= self.attack_duration * 1000:
                ...
                self.intervalo_animacao = self.intervalo_animacao_andar
                self.sprite_index = 0
                self.tempo_ultimo_update_animacao = agora # Resetar timer da animação de andar/idle
                self.attack_hitbox.size = (0,0)
        else:
            if jogador_valido:
            ...
            if not self.is_attacking and self.velocidade > 0: # Só se move se tiver velocidade e não estiver atacando
                if jogador_valido: # Só se move se houver um jogador válido para seguir
                    self.mover_em_direcao(player.rect.centerx, player.rect.centery, dt_ms)

            self.atualizar_animacao() # Chama a animação da InimigoBase ou a sobrescrita

        # Dano de contato (herdado de InimigoBase se lá implementado, ou aqui)
        if jogador_valido and self.rect.colliderect(player.rect) and \
           (agora - self.last_contact_time >= self.contact_cooldown):
            # print(f"DEBUG(Planta_Carnivora): Colisão de contato com jogador. Rect: {self.rect}, Player rect: {player.rect}")
            player.receber_dano(self.contact_damage, self.rect) # Passar self.rect como fonte_dano_rect
    ...
    def receber_dano(self, dano, fonte_dano_rect=None):
        vida_antes = self.hp
        super().receber_dano(dano, fonte_dano_rect) # Chama o receber_dano da InimigoBase
        if self.esta_vivo():
            if vida_antes > self.hp and Planta_Carnivora.som_dano_planta:
                Planta_Carnivora.som_dano_planta.play()
            # O flash de dano é acionado no placeholder da InimigoBase pela chamada a super().receber_dano()
            # Ou na sua implementação real da InimigoBase.
        elif vida_antes > 0 and Planta_Carnivora.som_morte_planta: # Morreu agora
