    ...
    class InimigoBase(pygame.sprite.Sprite):
        ...
        def mover_em_direcao(self, ax, ay, dt_ms=None):
            ...
            if dist > 0 and self.velocidade > 0:
                dx_norm = dx / dist
                dy_norm = dy / dist
                # Ajuste de movimento baseado em dt_ms para consistência de velocidade
                # Assumindo que self.velocidade é pixels por "frame ideal" (ex: 1/60 de segundo)
                mov_x = dx_norm * self.velocidade * (dt_ms / (1000.0 / 60.0))
                mov_y = dy_norm * self.velocidade * (dt_ms / (1000.0 / 60.0))
                self.rect.x += mov_x
                self.rect.y += mov_y
        ...
        def atualizar_animacao(self):
            ...
            if self.sprites and len(self.sprites) > 1: # Anima apenas se houver mais de um sprite
                if agora - self.tempo_ultimo_update_animacao > self.intervalo_animacao:
                    self.tempo_ultimo_update_animacao = agora
                    self.sprite_index = (self.sprite_index + 1) % len(self.sprites)
                    self.image = self.sprites[self.sprite_index]
            elif self.sprites and len(self.sprites) == 1: # Se houver apenas um sprite, usa ele
                 self.image = self.sprites[0]
            # Aplica flip se necessário
            if hasattr(self, 'image') and self.image and hasattr(self, 'facing_right') and not self.facing_right:
        ...
        def update(self, player, outros_inimigos=None, projeteis_inimigos_ref=None, tela_largura=None, altura_tela=None, dt_ms=None):
            ...
            # A lógica de movimento e ataque seria implementada na classe filha (Planta_Carnivora)
...
class Planta_Carnivora(InimigoBase):
    ...
    def __init__(self, x, y, velocidade=0.5): # Plantas carnívoras podem ser lentas ou imóveis
        ...
        xp_planta = 50
        self.moedas_drop = 15 # Quantidade de moedas que a Planta Carnívora dropa
        ...
        if not (hasattr(self, 'image') and isinstance(self.image, pygame.Surface)) or \
           (self.sprites and len(self.sprites) > 0 and self.image is self.sprites[0] and self.sprites[0].get_size() != Planta_Carnivora.tamanho_sprite_definido):
